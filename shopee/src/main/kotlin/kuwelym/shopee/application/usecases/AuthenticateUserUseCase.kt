...
import kuwelym.shopee.application.dto.LoginResponse
        ...
        return try {
            ...
            if (buyer != null) {
                if (passwordService.matches(request.password, buyer.password)) {
                    val token = tokenService.generateToken(buyer.username)
            ...
            if (shop != null) {
                if (passwordService.matches(request.password, shop.password)) {
                    val token = tokenService.generateToken(shop.username)
                    return Result.success(
                        LoginResponse(
                            token = token,
                            userType = "SHOP",
